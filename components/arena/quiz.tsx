...
import { Question } from "@/types/arena";
...
export default function ArenaQuiz({
  ...
  const handleSelect = (option: string) => {
    ...
    // 🔥 Send result to parent (XP, streak system later)
    onAnswer(isCorrect);
  };
          ...
          {current.options.map((option, i) => {
            ...
            return (
              <button
                ...
                className={`text-left px-4 py-2 rounded-xl border transition-all
                  ${
                    showAnswer
                      ? isCorrect
